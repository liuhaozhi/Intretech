 ...
 */
define(['N/record', 'N/search', 'N/url', 'N/https', 'N/ui/message', 'N/ui/dialog'],
    /**
     * @param {record} record
     * @param {search} search
     */
    function(record, search, url, https, message, dialog) {
        var popup, itemExist = {};
            ...
            searchBomBtn.onclick = searchBomBtn.onclick || function() {
                popup = popup || createPopup({
                    ...
                    gridConfig: {
                        ...
                        height: 400,
                        columns: [
                                    { label: "BOM 编号", fieldId: "cstm_bom_code_name", type: "hidden" },
                                    { label: "BOM 编号", fieldId: "cstm_bom_code_name_display", width: 200 },
                                    { label: "装配件编号", fieldId: "cstm_assembly_name", type: "hidden" },
                                    { label: "装配件编号", fieldId: "cstm_assembly_name_display", type: "hidden" },
                                    ...
                                    { label: "产出率", fieldId: "cstm_bom_output", type: "hidden" },
                                    { label: "位号", fieldId: "cstm_bom_location_number", type: "hidden" },
                                    { label: "版本号", fieldId: "cstm_revision_name", width: 100 },
                                    { label: "活动状态", fieldId: "cstm_bom_active_status", type: "hidden" },
                                    { label: "审核状态", fieldId: "cstm_approve_status", type: "hidden" },
                                    { label: "审核状态", fieldId: "cstm_approve_status_display", type: "hidden" },
                                    { label: "审核时间", fieldId: "cstm_approve_time", type: "hidden" },
                                    { label: "审核时间", fieldId: "cstm_approve_time_display", type: "hidden" },
                                ],
                        data: []
                    },
                    filters: [
        ...
        function createPopup(option) {
            ...
            var schGrid = new SearchListGrid(option.gridConfig);
            ...
            function searchComponentContent() {
                ...
                var gridConfig = {
                    ...
                    columns: [
                                ...
                                { label: "物料代码", fieldId: "cstm_bom_code", type: "hidden" },
                                { label: "物料代码", fieldId: "cstm_bom_code_display", width: 150, type: "text", data: [], onchange: bomCodeChange },
                                { label: "名称", fieldId: "cstm_bom_name", type: "hidden" },
                                { label: "规格", fieldId: "cstm_bom_specification" },
                                { label: "英文规格", fieldId: "cstm_bom_en_specification" },
                                { label: "物料属性", fieldId: "cstm_bom_attribute", type: "hidden" },
                                { label: "物料属性", fieldId: "cstm_bom_attribute_display" },
                                { label: "用量", fieldId: "cstm_bom_quantity", type: "text", onchange: checkQtyAndPosNum },
                                { label: "产出率", fieldId: "cstm_bom_output", type: "text" },
                                { label: "位号", fieldId: "cstm_bom_position", type: "text", onchange: checkQtyAndPosNum },
                                { label: "备注", fieldId: "cstm_bom_memo", type: "text" }
                            ],
                    data: [],
                ...
                var aGrid = new SearchListGrid(aGridConfig);
                var dGridConfig = deepCopy(gridConfig);
                dGridConfig.data.push(["Delete", "", "", "", "", "", "", "", "", "", "", "", "", ""]);
                var dGrid = new SearchListGrid(deepCopy(dGridConfig));
                var mConfige = deepCopy(gridConfig);
                mConfige.data = [];
                mConfige.data.push(["From", "", "", "", "", "", "", "", "", "", "", "", "", ""], ["To", "", "", "", "", "", "", "", "", "", "", "", "", ""]);
                var mGrid = new SearchListGrid(mConfige);
                return {
            ...
            function getActiveGridIntance() {
                ...
                if(activeTabe) {
                    activeTabe = document.querySelector("." + activeTabe.id + "_content").firstElementChild;
        ...
        function SearchListGrid(option) {
            ...
            var notDataLine = '<tr><td class="uir-nodata-row listtexthl" colspan="14">无数据可显示.</td></tr>';
            beforeMouseWheelEvent = beforeMouseWheelEvent || window.onmousewheel;
            lineHeight = 20, headerHeight = 24;
            gridNode.style = "max-height:" + height + "px;height:auto;overflow:hidden;position:relative;padding: 5px 0 10px 0;";
            gridNode.id = gridId;
            ...
            function gridClickEvent(el) {
                ...
                if(lastTarget != tdNode){
                    ...
                    if(i > -1) {
                        ...
                        if(_columns[pos] === undefined || (typeof option.cellEdit == "function" && !option.cellEdit(machine, tdNode, _columns[pos]))) { return; }
