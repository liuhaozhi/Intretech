    ...
    function validateQuantity(context) {
        ...
        for (var i = 0; i < lineCount; i++) {
            currentItemObj = {
                custpage_item: '',
                custpage_quantity: '',
                custpage_quantity_pushed_down: 0,
            ...
            for (var key in currentItemObj) {
                if (currentItemObj.hasOwnProperty(key)) {
                    ...
                    // if (key == 'custpage_item') {
                    //     currentItemObj[key] = pageRec.getSublistText({
                    //         sublistId: sublistId,
                    //         fieldId: key,
                    //         line: i
                    //     });
        ...
        for (var i = 0; i < currentItemList.length; i++) {
            if (currentItemList[i].custpage_checked) {
                soQty = Number(currentItemList[i].custpage_quantity);
                npQty = Number(currentItemList[i].custpage_quantity_not_pushed_down);
                pdQty = Number(currentItemList[i].custpage_quantity_pushed_down);
