...
define([
    ...
    const currentStateScriptId = 'custscript_q_prpo_approve_state';
    const workflowId = 'customworkflow_pr_approval_routing';
    const actionId = 'workflowaction_pr_route_approve';
    ...
    function getInputData(context) {
        const paramFieldId = 'custscript_q_prpo_approve_data_source';
        const currentScript = runtimeMod.getCurrentScript();
    ...
    function summarize(summary) {
        ...
        mapSummary.errors.iterator().each((key, error, executionNo) => {
            log.error({
                title: `第${executionNo}次审批同意PR ${key}时出错`,
                details: error
            });
            return true;
        });
