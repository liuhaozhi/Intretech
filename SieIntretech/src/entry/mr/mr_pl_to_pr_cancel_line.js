...
define([
    ...
    function validateParent(parentRec) {
        ...
        if(planStatus != approvedStatus){//已审核的才能取消
    ...
    function getInputData(context) {
        const paramFieldId = 'custscript_q_cancel_data_source';
    ...
    function summarize(summary) {
        ...
        mapSummary.errors.iterator().each((key, error, executionNo) => {
            log.error({
                title: `第${executionNo}次取消计划单${key}时出错`,
                details: error
            });
            return true;
        });
