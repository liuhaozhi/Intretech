 ...
 *@description CUX-PR的分拆与合并-为下推标准PR和PO
 */
define([
    'N/record',
    'N/search',
    ...
    const createPoTaskScriptId = 'customscript_mr_pr_to_po_merge_transform';
    const createPoTaskScriptInputId = 'custscript_q_pr_po_merge_trans_source';
    ...
    function getInputData(context) {
        const paramFieldId = 'custscript_q_prpo_spme_datas';
            ...
            for (const [groupKey, prList] of mergeGroups) {
                try {
                    if (prList.length === 1) {//如果该组只有一个PR，则直接输出
                        ...
                        // const totalMsgs = prList.filter(({ message }) => !!message).map(({ message }) => message).join('.');
                        // mergeMessages.add(totalMsgs);
