...
define([
    ...
    function setK3PoLineInfo(valueList, pannedOrderSchIds) {
        if(!pannedOrderSchIds.length) { return; }
        var info = {};
        var plannedOrderSchResults = searchCommonMod.getAllSearchResults({
            searchDefine: {
                type: "customrecordmrp_planned_order",
                columns: ['custrecord_cux_mrp_k3_po_number', 'custrecord_cux_mrp_k3_po_line']
            },
            filters: [ "internalid", "anyof", pannedOrderSchIds ]
        });
        for(var i = 0; i < plannedOrderSchResults.length; i++) {
            var item = plannedOrderSchResults[i];
            info[item.id] = info[item.id] || {};
            info[item.id]["custrecord_cux_mrp_k3_po_number"] = item.getValue(item.columns[0]);
            info[item.id]["custrecord_cux_mrp_k3_po_line"] = item.getValue(item.columns[1]);
        }
        var values = valueList[0].values, fieldIdIndexMap = {};
        for(var fIdIndex = values.length - 1; fieldIdIndexMap[values[fIdIndex].data.fieldId] = fIdIndex; fIdIndex--);
        for(var line in valueList) {
            values = valueList[line].values;
            var pannedOrderId = values[fieldIdIndexMap["custpage_custrecord_plan_order_num_pr0"]].data.value;
            if(!info[pannedOrderId] && info[pannedOrderId] != 0) { continue; }
            values[fieldIdIndexMap["custpage_custrecord_cux_mrp_k3_po_number_pr0"]].data.value = info[pannedOrderId]["custrecord_cux_mrp_k3_po_number"];
            values[fieldIdIndexMap["custpage_custrecord_cux_mrp_k3_po_line_pr0"]].data.value = info[pannedOrderId]["custrecord_cux_mrp_k3_po_line"];
            values[fieldIdIndexMap["custpage_custrecord_cux_mrp_k3_po_number_pr0"]].data.text = info[pannedOrderId]["custrecord_cux_mrp_k3_po_number"];
            values[fieldIdIndexMap["custpage_custrecord_cux_mrp_k3_po_line_pr0"]].data.text = info[pannedOrderId]["custrecord_cux_mrp_k3_po_line"];
        }
    }
    ...
    function viewPage(request, response) {
        ...
        const formCSPath = '../cs/cs_pr_to_po_platform_rebuild';
