 ...
 *@description 此程序主要用于自定义PR的合并下推功能
 */
define([
    'N/search',
    'N/ui/serverWidget',
    ...
    const jobName = '合并下推';
    const taskScriptId = 'customscript_mr_pr_to_po_split_merge';
    const taskInputScriptId = 'custscript_q_prpo_spme_datas';
                            ...
                            if (secondTaskDetail) {
                                ...
                                if (stPrIdNumMap.size) {
                                    searchMod.create({
                                        ...
                                        stPrIdNumMap.set(String(id), num);
                                        return true;
                                    });
                                }
                                if (poIdNumMap.size) {
                                ...
                                for (const { lines: summaryGroup, subsidiaryId, vendorId, stPrId, poId, } of summaryObj) {
                                    ...
                                    for (const { prId, transformQty, message, } of summaryGroup) {
                                    ...
                                    tableContent += `<tr><td>${subsidiaryName}</td><td>${vendorName}</td><td>${poLink}</td><td>${poStatus}</td><td>${prNums}</td><td>${statusText}</td><td>${messages}</td></tr>`;
                                }
                                ...
                                html = `<div style="text-align:center;">
                                                ...
                                                #custpage_result_summary {
                                                    ...
                                                    <tr><th>子公司</th><th>供应商</th><th>采购订单号</th><th>PO状态</th><th>请购单号:下推数量</th><th>创建状态</th><th>提示信息</th></tr>
                                                    ${tableContent}
                                                </table>
                                            </div>`;
                            }
    ...
    function onRequest(context) {
        ...
        // const { submitPage } = appPlatformCommon;
